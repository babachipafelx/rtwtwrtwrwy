...
-- Shared role manager state used by ESP/Trigger/Hitbox logic.
...
local function getSharedPlayerRole(player)
    ...
    -- Fallback: check if player is whitelisted in Hitbox or Trigger bot UI options
    local isWhitelisted = false
    pcall(function()
        if Options then
            local hw = Options.HitboxWhitelist
            local tw = Options.TriggerWhitelist
            if hw and type(hw.Value) == 'table' then
                if hw.Value[name] or (displayName and hw.Value[displayName]) then
                    isWhitelisted = true
                end
            end
            if not isWhitelisted and tw and type(tw.Value) == 'table' then
...
local LeftGroupBox = Tabs.Main:AddLeftGroupbox('HitBox')
-- We can also get our Main tab via the following code:
-- local LeftGroupBox = Window.Tabs.Main:AddLeftGroupbox('Groupbox')

-- Tabboxes are a tiny bit different, but here's a basic example:
-- HitBox controls: enable, show, color picker and size slider
LeftGroupBox:AddToggle('HitboxEnable', {
    Text = 'Enable Hitboxes',
    Default = false,
    Tooltip = 'Enable hitbox expansion for other players'
})

LeftGroupBox:AddToggle('HitboxShow', {
    Text = 'Show Hitbox Outline',
    Default = false,
    Tooltip = 'Show hitbox outline via Drawing (no world adornments � adornments ban)'
})

LeftGroupBox:AddLabel('Hitbox Color'):AddColorPicker('HitboxColor', {
    Default = Color3.fromRGB(37, 115, 184),
    Title = 'Hitbox Color',
    Transparency = 0,
    Callback = function(Value)
        -- handled below via Options hook
    end
})

LeftGroupBox:AddSlider('HitboxSizeX', {
    Text = 'Hitbox Size X (0 = normal)',
    Default = 0,
    Min = 0,
    Max = 50,
    Rounding = 0,
...
LeftGroupBox:AddSlider('HitboxSizeY', {
    Text = 'Hitbox Size Y (0 = normal)',
    Default = 0,
    Min = 0,
    Max = 50,
    Rounding = 0,
...
LeftGroupBox:AddSlider('HitboxSizeZ', {
    Text = 'Hitbox Size Z (0 = normal)',
    Default = 0,
    Min = 0,
    Max = 50,
    Rounding = 0,
...
local HitboxSizeX = 0 -- 0 = normal
local HitboxSizeY = 0 -- 0 = normal
local HitboxSizeZ = 0 -- 0 = normal
local HitboxColor = Color3.fromRGB(37, 115, 184)
local DBHitboxEnabled = false
local DBHitboxSizeX = 0
local DBHitboxSizeY = 0
local DBHitboxSizeZ = 0
local DBHitboxColor = Color3.fromRGB(37, 115, 184)
local RevolverHitboxEnabled = false
local RevolverHitboxSizeX = 0
local RevolverHitboxSizeY = 0
local RevolverHitboxSizeZ = 0
local RevolverHitboxColor = Color3.fromRGB(37, 115, 184)
local ShotgunHitboxEnabled = false
local ShotgunHitboxSizeX = 0
local ShotgunHitboxSizeY = 0
local ShotgunHitboxSizeZ = 0
local ShotgunHitboxColor = Color3.fromRGB(37, 115, 184)
local TacticalShotgunHitboxEnabled = false
local TacticalShotgunHitboxSizeX = 0
local TacticalShotgunHitboxSizeY = 0
local TacticalShotgunHitboxSizeZ = 0
local TacticalShotgunHitboxColor = Color3.fromRGB(37, 115, 184)
local OrigSizes = {}
local HitboxWhitelist = {} -- keys are player names to exclude from hitbox changes

-- Drawing-only hitbox visuals. BoxHandleAdornment/SelectionBox = ExamsAC esp_adornment ban.
local HitboxDrawings = {} -- [userId] = { Drawing.Line x12 }
local HITBOX_EDGE_PAIRS = {
    { 1, 2 }, { 2, 3 }, { 3, 4 }, { 4, 1 },
    { 5, 6 }, { 6, 7 }, { 7, 8 }, { 8, 5 },
    { 1, 5 }, { 2, 6 }, { 3, 7 }, { 4, 8 },
}

local function destroyHitboxDrawing(userId)
    local lines = HitboxDrawings[userId]
    if not lines then
        return
    end
    ...
    HitboxDrawings[userId] = nil
end

local function clearAllHitboxDrawings()
    for userId in pairs(HitboxDrawings) do
        destroyHitboxDrawing(userId)
    end
end

local function scrubHitboxAdornments(hrp)
    if not hrp then
        return
    end
    for _, name in ipairs({ 'HitboxOutline', 'ForcefieldEffect' }) do
    ...
    for _, ch in ipairs(hrp:GetChildren()) do
        if ch:IsA('BoxHandleAdornment') or ch:IsA('SelectionBox')
            or ch:IsA('CylinderHandleAdornment') or ch:IsA('SphereHandleAdornment') then
            pcall(function() ch:Destroy() end)
        end
    end
end

local function ensureHitboxLines(userId)
    local lines = HitboxDrawings[userId]
    if lines and #lines == 12 then
        return lines
    end
    if lines then
        destroyHitboxDrawing(userId)
    ...
    for i = 1, 12 do
        ...
        if not ok or not ln then
            destroyHitboxDrawing(userId)
            return nil
        end
        lines[i] = ln
    end
    HitboxDrawings[userId] = lines
    return lines
end

local function updateHitboxDrawing(userId, hrp, color)
    local cam = workspace.CurrentCamera
    if not cam or not hrp or not hrp.Parent then
        destroyHitboxDrawing(userId)
        return
    end
    local lines = ensureHitboxLines(userId)
    if not lines then
        return
    end

    local cf = hrp.CFrame
    local half = hrp.Size * 0.5
    local corners = {
    ...
    local screen = table.create(8)
    local front = table.create(8)
    for i = 1, 8 do
        local sp = cam:WorldToViewportPoint(corners[i])
        screen[i] = Vector2.new(sp.X, sp.Y)
        front[i] = sp.Z > 0
    end

    local col = color or HitboxColor
    for ei, pair in ipairs(HITBOX_EDGE_PAIRS) do
        local ln = lines[ei]
...
local function hideHitboxDrawing(userId)
    local lines = HitboxDrawings[userId]
    if not lines then
        return
    end
...
-- Helper: check if player is KO'd. Attempts to find BodyEffects under workspace.Players.Name or the character.
...
-- Wire up UI -> state
Toggles.HitboxEnable:OnChanged(function()
    HitboxEnabled = Toggles.HitboxEnable.Value
    if not HitboxEnabled then
        clearAllHitboxDrawings()
        for _, pl in ipairs(game:GetService('Players'):GetPlayers()) do
            if pl.Character then
                local hrp = pl.Character:FindFirstChild('HumanoidRootPart')
                if hrp then
                    scrubHitboxAdornments(hrp)
                    if OrigSizes[pl.UserId] then
                        pcall(function() hrp.Size = OrigSizes[pl.UserId] end)
                        OrigSizes[pl.UserId] = nil
                    end
                end
            end
        end
...
Toggles.HitboxShow:OnChanged(function()
    HitboxShow = Toggles.HitboxShow.Value
    if not HitboxShow then
        clearAllHitboxDrawings()
    end
end)

Options.HitboxColor:OnChanged(function()
    HitboxColor = Options.HitboxColor.Value
end)

Options.HitboxSizeX:OnChanged(function()
    HitboxSizeX = Options.HitboxSizeX.Value
end)

Options.HitboxSizeY:OnChanged(function()
    HitboxSizeY = Options.HitboxSizeY.Value
end)

Options.HitboxSizeZ:OnChanged(function()
    HitboxSizeZ = Options.HitboxSizeZ.Value
end)

Options.DBHitboxColor = makeOption('DBHitboxColor', Color3.fromRGB(37, 115, 184))
Options.DBHitboxEnabled = makeToggle('DBHitboxEnabled', false)
Options.DBHitboxSizeX = makeOption('DBHitboxSizeX', 0)
Options.DBHitboxSizeY = makeOption('DBHitboxSizeY', 0)
Options.DBHitboxSizeZ = makeOption('DBHitboxSizeZ', 0)

Toggles.DBHitboxEnabled = Options.DBHitboxEnabled

Options.DBHitboxColor:OnChanged(function()
    DBHitboxColor = Options.DBHitboxColor.Value
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Options.DBHitboxSizeX:OnChanged(function()
    DBHitboxSizeX = Options.DBHitboxSizeX.Value
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Options.DBHitboxSizeY:OnChanged(function()
    DBHitboxSizeY = Options.DBHitboxSizeY.Value
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Options.DBHitboxSizeZ:OnChanged(function()
    DBHitboxSizeZ = Options.DBHitboxSizeZ.Value
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Toggles.DBHitboxEnabled:OnChanged(function()
    DBHitboxEnabled = Toggles.DBHitboxEnabled.Value == true
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Options.RevolverHitboxColor = makeOption('RevolverHitboxColor', Color3.fromRGB(37, 115, 184))
Options.RevolverHitboxEnabled = makeToggle('RevolverHitboxEnabled', false)
Options.RevolverHitboxSizeX = makeOption('RevolverHitboxSizeX', 0)
Options.RevolverHitboxSizeY = makeOption('RevolverHitboxSizeY', 0)
Options.RevolverHitboxSizeZ = makeOption('RevolverHitboxSizeZ', 0)

Toggles.RevolverHitboxEnabled = Options.RevolverHitboxEnabled

Options.RevolverHitboxColor:OnChanged(function()
    RevolverHitboxColor = Options.RevolverHitboxColor.Value
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Options.RevolverHitboxSizeX:OnChanged(function()
    RevolverHitboxSizeX = Options.RevolverHitboxSizeX.Value
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Options.RevolverHitboxSizeY:OnChanged(function()
    RevolverHitboxSizeY = Options.RevolverHitboxSizeY.Value
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Options.RevolverHitboxSizeZ:OnChanged(function()
    RevolverHitboxSizeZ = Options.RevolverHitboxSizeZ.Value
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Toggles.RevolverHitboxEnabled:OnChanged(function()
    RevolverHitboxEnabled = Toggles.RevolverHitboxEnabled.Value == true
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Options.ShotgunHitboxColor = makeOption('ShotgunHitboxColor', Color3.fromRGB(37, 115, 184))
Options.ShotgunHitboxEnabled = makeToggle('ShotgunHitboxEnabled', false)
Options.ShotgunHitboxSizeX = makeOption('ShotgunHitboxSizeX', 0)
Options.ShotgunHitboxSizeY = makeOption('ShotgunHitboxSizeY', 0)
Options.ShotgunHitboxSizeZ = makeOption('ShotgunHitboxSizeZ', 0)

Toggles.ShotgunHitboxEnabled = Options.ShotgunHitboxEnabled

Options.ShotgunHitboxColor:OnChanged(function()
    ShotgunHitboxColor = Options.ShotgunHitboxColor.Value
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Options.ShotgunHitboxSizeX:OnChanged(function()
    ShotgunHitboxSizeX = Options.ShotgunHitboxSizeX.Value
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Options.ShotgunHitboxSizeY:OnChanged(function()
    ShotgunHitboxSizeY = Options.ShotgunHitboxSizeY.Value
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Options.ShotgunHitboxSizeZ:OnChanged(function()
    ShotgunHitboxSizeZ = Options.ShotgunHitboxSizeZ.Value
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Toggles.ShotgunHitboxEnabled:OnChanged(function()
    ShotgunHitboxEnabled = Toggles.ShotgunHitboxEnabled.Value == true
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Options.TacticalShotgunHitboxColor = makeOption('TacticalShotgunHitboxColor', Color3.fromRGB(37, 115, 184))
Options.TacticalShotgunHitboxEnabled = makeToggle('TacticalShotgunHitboxEnabled', false)
Options.TacticalShotgunHitboxSizeX = makeOption('TacticalShotgunHitboxSizeX', 0)
Options.TacticalShotgunHitboxSizeY = makeOption('TacticalShotgunHitboxSizeY', 0)
Options.TacticalShotgunHitboxSizeZ = makeOption('TacticalShotgunHitboxSizeZ', 0)

Toggles.TacticalShotgunHitboxEnabled = Options.TacticalShotgunHitboxEnabled

Options.TacticalShotgunHitboxColor:OnChanged(function()
    TacticalShotgunHitboxColor = Options.TacticalShotgunHitboxColor.Value
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Options.TacticalShotgunHitboxSizeX:OnChanged(function()
    TacticalShotgunHitboxSizeX = Options.TacticalShotgunHitboxSizeX.Value
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Options.TacticalShotgunHitboxSizeY:OnChanged(function()
    TacticalShotgunHitboxSizeY = Options.TacticalShotgunHitboxSizeY.Value
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Options.TacticalShotgunHitboxSizeZ:OnChanged(function()
    TacticalShotgunHitboxSizeZ = Options.TacticalShotgunHitboxSizeZ.Value
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

Toggles.TacticalShotgunHitboxEnabled:OnChanged(function()
    TacticalShotgunHitboxEnabled = Toggles.TacticalShotgunHitboxEnabled.Value == true
    if type(scheduleHitboxRefresh) == 'function' then
        scheduleHitboxRefresh()
    end
end)

-- Legacy Whitelist dropdown stubbed out for safety
Options.HitboxWhitelist = makeOption('HitboxWhitelist', {})

-- Ensure slider starts at 0
Options.HitboxSizeX:SetValue(0)
Options.HitboxSizeY:SetValue(0)
Options.HitboxSizeZ:SetValue(0)
Options.DBHitboxSizeX:SetValue(0)
Options.DBHitboxSizeY:SetValue(0)
Options.DBHitboxSizeZ:SetValue(0)
Options.RevolverHitboxSizeX:SetValue(0)
Options.RevolverHitboxSizeY:SetValue(0)
Options.RevolverHitboxSizeZ:SetValue(0)
Options.ShotgunHitboxSizeX:SetValue(0)
Options.ShotgunHitboxSizeY:SetValue(0)
Options.ShotgunHitboxSizeZ:SetValue(0)
Options.TacticalShotgunHitboxSizeX:SetValue(0)
Options.TacticalShotgunHitboxSizeY:SetValue(0)
Options.TacticalShotgunHitboxSizeZ:SetValue(0)

local localDbWeaponEquipped = false
local localRevolverWeaponEquipped = false
local localShotgunWeaponEquipped = false
local localTacticalShotgunWeaponEquipped = false
local hitboxRefreshQueued = false

local function localWeaponMatchesDoubleBarrel()
    local players = game:GetService('Players')
    local localPlayer = players.LocalPlayer
    ...
    local name = tool and tostring(tool.Name or '') or ''
    local lowered = string.lower(name)
    return lowered == '[double-barrel sg]'
...
local function localWeaponMatchesRevolver()
    local players = game:GetService('Players')
    local localPlayer = players.LocalPlayer
    if not localPlayer or not localPlayer.Character then
        return false
    ...
    return lowered == '[revolver]'
        or lowered == 'revolver'
        or string.find(lowered, 'revolver', 1, false) ~= nil
end

local function localWeaponMatchesTacticalShotgun()
    local players = game:GetService('Players')
    local localPlayer = players.LocalPlayer
    if not localPlayer or not localPlayer.Character then
        return false
    ...
    local name = tool and tostring(tool.Name or '') or ''
    local lowered = string.lower(name)
    return lowered == '[tacticalshotgun]'
        or lowered == '[tactical shotgun]'
        or (string.find(lowered, 'tactical', 1, false) ~= nil and string.find(lowered, 'shotgun', 1, false) ~= nil)
end

local function localWeaponMatchesShotgun()
    if localWeaponMatchesTacticalShotgun() then
        return false
    end
    local players = game:GetService('Players')
    local localPlayer = players.LocalPlayer
    ...
    local name = tool and tostring(tool.Name or '') or ''
    local lowered = string.lower(name)
    return lowered == '[shotgun]'
        or lowered == 'shotgun'
        or string.find(lowered, 'shotgun', 1, false) ~= nil
end

local function syncLocalWeaponStates()
    local dbEquipped = localWeaponMatchesDoubleBarrel()
    local revEquipped = localWeaponMatchesRevolver()
    local sgEquipped = localWeaponMatchesShotgun()
    local tacEquipped = localWeaponMatchesTacticalShotgun()
    local changed = false
    if dbEquipped ~= localDbWeaponEquipped then
        localDbWeaponEquipped = dbEquipped
        changed = true
    end
    if revEquipped ~= localRevolverWeaponEquipped then
        localRevolverWeaponEquipped = revEquipped
        changed = true
    end
    if sgEquipped ~= localShotgunWeaponEquipped then
        localShotgunWeaponEquipped = sgEquipped
        changed = true
    end
    if tacEquipped ~= localTacticalShotgunWeaponEquipped then
        localTacticalShotgunWeaponEquipped = tacEquipped
        changed = true
    end
    return changed
end

local function getActiveHitboxConfig()
    if TacticalShotgunHitboxEnabled and localTacticalShotgunWeaponEquipped then
        return TacticalShotgunHitboxSizeX, TacticalShotgunHitboxSizeY, TacticalShotgunHitboxSizeZ, TacticalShotgunHitboxColor
    end
    if ShotgunHitboxEnabled and localShotgunWeaponEquipped then
        return ShotgunHitboxSizeX, ShotgunHitboxSizeY, ShotgunHitboxSizeZ, ShotgunHitboxColor
    end
    if RevolverHitboxEnabled and localRevolverWeaponEquipped then
        return RevolverHitboxSizeX, RevolverHitboxSizeY, RevolverHitboxSizeZ, RevolverHitboxColor
    end
    if DBHitboxEnabled and localDbWeaponEquipped then
        return DBHitboxSizeX, DBHitboxSizeY, DBHitboxSizeZ, DBHitboxColor
    end
    return HitboxSizeX, HitboxSizeY, HitboxSizeZ, HitboxColor
end

local processHitboxesOnce
local scheduleHitboxRefresh

scheduleHitboxRefresh = function()
    if hitboxRefreshQueued or not HitboxEnabled then
        return
    end
    hitboxRefreshQueued = true
    task.defer(function()
        hitboxRefreshQueued = false
        if HitboxEnabled and type(processHitboxesOnce) == 'function' then
            processHitboxesOnce()
        end
    end)
end

local function bindLocalWeaponWatchers()
    local Players = game:GetService('Players')
    local localPlayer = Players.LocalPlayer
    ...
    local function onToolChanged(child)
        ...
        if syncLocalWeaponStates() then
            scheduleHitboxRefresh()
        end
    end

    local function onCharacter(character)
        syncLocalWeaponStates()
...
bindLocalWeaponWatchers()

-- Main worker loop
safeSpawn(function()
    local Players = game:GetService('Players')
    local hitboxClean = true

    processHitboxesOnce = function()
        local sizeX, sizeY, sizeZ, activeColor = getActiveHitboxConfig()
        for _, pl in ipairs(Players:GetPlayers()) do
            pcall(function()
                local localPlayer = Players.LocalPlayer
                if pl == localPlayer then return end

                if isSharedFriendRole(pl)
                    or (HitboxWhitelist[pl.Name] and not isSharedTargetRole(pl))
                    or (Toggles.HitboxTargetOnly and Toggles.HitboxTargetOnly.Value == true and not isSharedTargetRole(pl)) then
                    if pl.Character then
                        local hrp = pl.Character:FindFirstChild('HumanoidRootPart')
                        if hrp then
                            scrubHitboxAdornments(hrp)
                            hideHitboxDrawing(pl.UserId)
                            if OrigSizes[pl.UserId] then
                                pcall(function() hrp.Size = OrigSizes[pl.UserId] end)
                                OrigSizes[pl.UserId] = nil
                            end
                ...
                local isKO = IsPlayerKO(pl)

                if isKO then
                    if not OrigSizes[pl.UserId] then OrigSizes[pl.UserId] = hrp.Size end
                    ...
                    if sizeX > 0 or sizeY > 0 or sizeZ > 0 then
                        if not OrigSizes[pl.UserId] then OrigSizes[pl.UserId] = hrp.Size end
                        hrp.CanCollide = false
                        local ox, oy, oz = OrigSizes[pl.UserId].X, OrigSizes[pl.UserId].Y, OrigSizes[pl.UserId].Z
                        hrp.Size = Vector3.new(sizeX > 0 and sizeX or ox, sizeY > 0 and sizeY or oy, sizeZ > 0 and sizeZ or oz)
                    else
                        if OrigSizes[pl.UserId] then
                            hrp.Size = OrigSizes[pl.UserId]
                            OrigSizes[pl.UserId] = nil
                        end
                    end
                end

                -- Never create adornments (ExamsAC). Scrub leftovers + draw via Drawing API.
                scrubHitboxAdornments(hrp)
                if (not isKO) and HitboxShow and (sizeX > 0 or sizeY > 0 or sizeZ > 0) then
                    updateHitboxDrawing(pl.UserId, hrp, activeColor)
                else
                    hideHitboxDrawing(pl.UserId)
                end
            end)
    ...
    safeConnect(game:GetService('RunService').RenderStepped, function()
        if Library.Unloaded or not HitboxEnabled or not HitboxShow then
            return
        end
        local sizeX, sizeY, sizeZ, activeColor = getActiveHitboxConfig()
        if not (sizeX > 0 or sizeY > 0 or sizeZ > 0) then
            return
        end
        for _, pl in ipairs(Players:GetPlayers()) do
            if pl ~= Players.LocalPlayer and pl.Character and not IsPlayerKO(pl) then
                if not (isSharedFriendRole(pl)
                    or (HitboxWhitelist[pl.Name] and not isSharedTargetRole(pl))
                    or (Toggles.HitboxTargetOnly and Toggles.HitboxTargetOnly.Value == true and not isSharedTargetRole(pl))) then
                    local hrp = pl.Character:FindFirstChild('HumanoidRootPart')
                    if hrp then
    ...
    safeConnect(Players.PlayerRemoving, function(pl)
        if pl then
            destroyHitboxDrawing(pl.UserId)
        end
    end)
    ...
    while true do
        ...
        if HitboxEnabled then
            ...
            processHitboxesOnce()
            local waitTime = 0.7
            if DBHitboxEnabled or RevolverHitboxEnabled or ShotgunHitboxEnabled or TacticalShotgunHitboxEnabled then
                waitTime = (localDbWeaponEquipped or localRevolverWeaponEquipped or localShotgunWeaponEquipped or localTacticalShotgunWeaponEquipped) and 0.25 or 0.45
            end
            task.wait(waitTime)
        else
            if not hitboxClean then
                clearAllHitboxDrawings()
                for _, pl in ipairs(Players:GetPlayers()) do
                    pcall(function()
                        if not pl.Character then return end
...
-- ?????????????????????????????????????????????
--  VEST FIX MODULE (DefenseBBGUI.Vest + hitbox HRP)
-- ?????????????????????????????????????????????
LeftGroupBox:AddToggle('VestFixEnable', { Text = 'Vest Fix', Default = false })
    ...
    if VisualsTab then
        ...
        safeSpawn(function()
            ...
            local function getPlayerRole(player)
                ...
                if isInSelection('HitboxWhitelist', player) or isInSelection('TriggerWhitelist', player) then
                    return 'Friend'
                end
                return 'Neutral'
            end
...
do
    ...
    local LEFT_HITBOX_PANEL_GAP = 6
    local MAX_LEFT_HITBOX_PANELS = 2
    local leftHitboxPanelList = { dbHitboxUi, revHitboxUi, sgHitboxUi, tacHitboxUi }
    local syncLeftHitboxPanels
    local prepareLeftHitboxPanelOpen
    ...
    applyTheme = function()
        ...
        if dbHitboxUi.gradient then
            dbHitboxUi.gradient.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, palette.surface),
                ColorSequenceKeypoint.new(1, palette.bg),
            })
        end
        if revHitboxUi.gradient then
            revHitboxUi.gradient.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, palette.surface),
                ColorSequenceKeypoint.new(1, palette.bg),
            })
        end
        if sgHitboxUi.gradient then
            sgHitboxUi.gradient.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, palette.surface),
                ColorSequenceKeypoint.new(1, palette.bg),
            })
        end
        if tacHitboxUi.gradient then
            tacHitboxUi.gradient.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, palette.surface),
                ColorSequenceKeypoint.new(1, palette.bg),
            })
        end
    ...
    local State = {}
    do
    State.HitboxEnable = ensureToggle('HitboxEnable', false)
    State.HitboxShow = ensureToggle('HitboxShow', false)
    State.HitboxTargetOnly = ensureToggle('HitboxTargetOnly', false)
    ...
    State.HitboxSizeX = ensureOption('HitboxSizeX', 0)
    State.HitboxSizeY = ensureOption('HitboxSizeY', 0)
    State.HitboxSizeZ = ensureOption('HitboxSizeZ', 0)
    State.HitboxWhitelist = ensureOption('HitboxWhitelist', {})
    ...
    State.HitboxColor = ensureOption('HitboxColor', Color3.fromRGB(37, 115, 184))
...
trackKeybindConnection(safeConnect(UIS.InputChanged, function(input)
    ...
    prepareLeftHitboxPanelOpen = function(targetUi)
        local openOthers = 0
        for _, ui in ipairs(leftHitboxPanelList) do
            if ui ~= targetUi and ui.open then
                openOthers = openOthers + 1
            end
        end
        if openOthers >= MAX_LEFT_HITBOX_PANELS then
            for _, ui in ipairs(leftHitboxPanelList) do
                if ui ~= targetUi and ui.open and ui.setVisible then
                    ui.setVisible(false)
                    break
                end
            end
    ...
    syncLeftHitboxPanels = function()
        if not main then
            return
        end
        local yOffset = main.Position.Y.Offset
        for _, ui in ipairs(leftHitboxPanelList) do
            if ui.open and ui.panel then
                local panel = ui.panel
                panel.Position = UDim2.new(
                    main.Position.X.Scale,
                    main.Position.X.Offset - panel.Size.X.Offset - LEFT_HITBOX_PANEL_GAP,
                    main.Position.Y.Scale,
                    yOffset
                )
                yOffset = yOffset + panel.Size.Y.Offset + LEFT_HITBOX_PANEL_GAP
            end
        end
    end
...
safeConnect(closeBtn.MouseLeave, function()
    ...
    do
        local panel = Instance.new('Frame')
        panel.Name = 'DBHitboxPanel'
        panel.Size = UDim2.fromOffset(248, 320)
        panel.BackgroundColor3 = palette.bg
        panel.BorderSizePixel = 0
        panel.Visible = false
        panel.Parent = menuGroup
        ...
        dbHitboxUi.syncSize = function()
            local padTop = pad.PaddingTop.Offset
            local padBottom = pad.PaddingBottom.Offset
            local contentH = layout.AbsoluteContentSize.Y
            local totalH = BODY_TOP + padTop + contentH + padBottom + 6
            panel.Size = UDim2.fromOffset(panel.Size.X.Offset, math.max(totalH, 220))
            syncLeftHitboxPanels()
        end

        dbHitboxUi.setVisible = function(show)
            if show then
                prepareLeftHitboxPanelOpen(dbHitboxUi)
            end
            dbHitboxUi.open = show == true
            panel.Visible = dbHitboxUi.open
            if dbHitboxUi.open then
                if dbHitboxUi.syncSize then
                    dbHitboxUi.syncSize()
                end
            end
            syncLeftHitboxPanels()
            if type(dbHitboxUi.refreshButton) == 'function' then
                dbHitboxUi.refreshButton()
            end
        end
    ...
    do
        local panel = Instance.new('Frame')
        panel.Name = 'RevolverHitboxPanel'
        panel.Size = UDim2.fromOffset(248, 320)
        panel.BackgroundColor3 = palette.bg
        panel.BorderSizePixel = 0
        panel.Visible = false
        ...
        revHitboxUi.setVisible = function(show)
            if show then
                prepareLeftHitboxPanelOpen(revHitboxUi)
            end
            revHitboxUi.open = show == true
            panel.Visible = revHitboxUi.open
            if revHitboxUi.open and revHitboxUi.syncSize then
                revHitboxUi.syncSize()
            end
            syncLeftHitboxPanels()
            if type(revHitboxUi.refreshButton) == 'function' then
                revHitboxUi.refreshButton()
            end
        end

        safeConnect(layout:GetPropertyChangedSignal('AbsoluteContentSize'), function()
            if revHitboxUi.syncSize then
                revHitboxUi.syncSize()
            end
        end)

        safeConnect(closeBtn.MouseButton1Click, function()
            revHitboxUi.setVisible(false)
        end)
    ...
    do
        ...
        sgHitboxUi.setVisible = function(show)
            if show then
                prepareLeftHitboxPanelOpen(sgHitboxUi)
            end
            sgHitboxUi.open = show == true
            panel.Visible = sgHitboxUi.open
            if sgHitboxUi.open and sgHitboxUi.syncSize then
                sgHitboxUi.syncSize()
            end
            syncLeftHitboxPanels()
            if type(sgHitboxUi.refreshButton) == 'function' then
                sgHitboxUi.refreshButton()
            end
        end

        safeConnect(layout:GetPropertyChangedSignal('AbsoluteContentSize'), function()
            if sgHitboxUi.syncSize then
                sgHitboxUi.syncSize()
            end
        end)

        safeConnect(closeBtn.MouseButton1Click, function()
            sgHitboxUi.setVisible(false)
        end)
    ...
    do
        local panel = Instance.new('Frame')
        panel.Name = 'TacticalShotgunHitboxPanel'
        panel.Size = UDim2.fromOffset(248, 320)
        panel.BackgroundColor3 = palette.bg
        panel.BorderSizePixel = 0
        panel.Visible = false
        ...
        tacHitboxUi.setVisible = function(show)
            if show then
                prepareLeftHitboxPanelOpen(tacHitboxUi)
            end
            tacHitboxUi.open = show == true
            panel.Visible = tacHitboxUi.open
            if tacHitboxUi.open and tacHitboxUi.syncSize then
                tacHitboxUi.syncSize()
            end
            syncLeftHitboxPanels()
            if type(tacHitboxUi.refreshButton) == 'function' then
                tacHitboxUi.refreshButton()
            end
        end

        safeConnect(layout:GetPropertyChangedSignal('AbsoluteContentSize'), function()
            if tacHitboxUi.syncSize then
                tacHitboxUi.syncSize()
            end
        end)

        safeConnect(closeBtn.MouseButton1Click, function()
            tacHitboxUi.setVisible(false)
        end)
...
trackConnection(safeConnect(UIS.InputBegan, function(input)
    ...
    ;(function()
        local hitboxSection = createSection(pages.Combat, 'Hitbox')
        createToggle(hitboxSection, 'Enable Hitboxes', State.HitboxEnable)
        createToggle(hitboxSection, 'Target Only', State.HitboxTargetOnly)
        createToggle(hitboxSection, 'Show Hitbox Outline', State.HitboxShow)
        createToggle(hitboxSection, 'Vest Fix', State.VestFixEnable)
        attachChangeListener(State.HitboxTargetOnly, function()
            if type(scheduleHitboxRefresh) == 'function' then
                scheduleHitboxRefresh()
            end
        end)
        createSlider(hitboxSection, 'Hitbox Size X', State.HitboxSizeX, 0, 50, 1, '')
        createSlider(hitboxSection, 'Hitbox Size Y', State.HitboxSizeY, 0, 50, 1, '')
        createSlider(hitboxSection, 'Hitbox Size Z', State.HitboxSizeZ, 0, 50, 1, '')
        createColorRow(hitboxSection, 'Hitbox Color', State.HitboxColor)
        do
            local ICON_SIZE = 38
            local ICON_GAP = 8

            local weaponIconRow = Instance.new('Frame')
            weaponIconRow.BackgroundTransparency = 1
            weaponIconRow.Size = UDim2.new(1, 0, 0, ICON_SIZE + 8)
            weaponIconRow.BorderSizePixel = 0
            weaponIconRow.Parent = hitboxSection

            local function createWeaponIconButton(imageId, xOffset, uiState)
                ...
                button.Parent = weaponIconRow
                applyCorner(button, 8)
                applyStroke(button, 'strokeSoft', 1, 0.45)
                ...
                local function refreshButton()
                    button.BackgroundColor3 = uiState.open and palette.accent or palette.surface
                    button.ImageColor3 = uiState.open and Color3.fromRGB(255, 255, 255) or palette.textDim
                end

                local function animatePress()
                    pressing = true
                    tween(buttonScale, 0.08, { Scale = 0.86 }, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
                end
                ...
                local function animateRelease()
                    ...
                    pressing = false
                    tween(buttonScale, 0.14, { Scale = 1 }, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
                end

                safeConnect(button.MouseButton1Down, animatePress)
                safeConnect(button.MouseButton1Up, animateRelease)
                safeConnect(button.MouseLeave, animateRelease)
                safeConnect(button.MouseButton1Click, function()
                    if uiState.setVisible then
                        uiState.setVisible(not uiState.open)
                    end
                    refreshButton()
                end)

                refreshButton()
                uiState.refreshButton = refreshButton
                registerThemeRefresher(refreshButton)
            end

            createWeaponIconButton('rbxassetid://102560347663140', 14, dbHitboxUi)
            createWeaponIconButton('rbxassetid://133591905030881', 14 + (ICON_SIZE + ICON_GAP), revHitboxUi)
            createWeaponIconButton('rbxassetid://15508805939', 14 + (ICON_SIZE + ICON_GAP) * 2, sgHitboxUi)
            createWeaponIconButton('rbxassetid://15508807160', 14 + (ICON_SIZE + ICON_GAP) * 3, tacHitboxUi)
        end
        if dbHitboxUi.body then
            createToggle(dbHitboxUi.body, 'Enable [Double-Barrel SG]', Toggles.DBHitboxEnabled)
            createSlider(dbHitboxUi.body, 'DB Hitbox Size X', Options.DBHitboxSizeX, 0, 50, 1, '')
            createSlider(dbHitboxUi.body, 'DB Hitbox Size Y', Options.DBHitboxSizeY, 0, 50, 1, '')
            createSlider(dbHitboxUi.body, 'DB Hitbox Size Z', Options.DBHitboxSizeZ, 0, 50, 1, '')
            createColorRow(dbHitboxUi.body, 'Hitbox Color', Options.DBHitboxColor)
            if dbHitboxUi.syncSize then
                runLater(0, dbHitboxUi.syncSize)
            end
        end
        if revHitboxUi.body then
            createToggle(revHitboxUi.body, 'Enable [Revolver]', Toggles.RevolverHitboxEnabled)
            createSlider(revHitboxUi.body, 'Revolver Hitbox Size X', Options.RevolverHitboxSizeX, 0, 50, 1, '')
            createSlider(revHitboxUi.body, 'Revolver Hitbox Size Y', Options.RevolverHitboxSizeY, 0, 50, 1, '')
            createSlider(revHitboxUi.body, 'Revolver Hitbox Size Z', Options.RevolverHitboxSizeZ, 0, 50, 1, '')
            createColorRow(revHitboxUi.body, 'Hitbox Color', Options.RevolverHitboxColor)
            if revHitboxUi.syncSize then
                runLater(0, revHitboxUi.syncSize)
            end
        end
        if sgHitboxUi.body then
            createToggle(sgHitboxUi.body, 'Enable [Shotgun]', Toggles.ShotgunHitboxEnabled)
            createSlider(sgHitboxUi.body, 'Shotgun Hitbox Size X', Options.ShotgunHitboxSizeX, 0, 50, 1, '')
            createSlider(sgHitboxUi.body, 'Shotgun Hitbox Size Y', Options.ShotgunHitboxSizeY, 0, 50, 1, '')
            createSlider(sgHitboxUi.body, 'Shotgun Hitbox Size Z', Options.ShotgunHitboxSizeZ, 0, 50, 1, '')
            createColorRow(sgHitboxUi.body, 'Hitbox Color', Options.ShotgunHitboxColor)
            if sgHitboxUi.syncSize then
                runLater(0, sgHitboxUi.syncSize)
            end
        end
        if tacHitboxUi.body then
            createToggle(tacHitboxUi.body, 'Enable [TacticalShotgun]', Toggles.TacticalShotgunHitboxEnabled)
            createSlider(tacHitboxUi.body, 'Tactical Shotgun Hitbox Size X', Options.TacticalShotgunHitboxSizeX, 0, 50, 1, '')
            createSlider(tacHitboxUi.body, 'Tactical Shotgun Hitbox Size Y', Options.TacticalShotgunHitboxSizeY, 0, 50, 1, '')
            createSlider(tacHitboxUi.body, 'Tactical Shotgun Hitbox Size Z', Options.TacticalShotgunHitboxSizeZ, 0, 50, 1, '')
            createColorRow(tacHitboxUi.body, 'Hitbox Color', Options.TacticalShotgunHitboxColor)
            if tacHitboxUi.syncSize then
                runLater(0, tacHitboxUi.syncSize)
            end
        end
    end)()
    ...
    ;(function()
        ...
        local function setRoleForPlayer(player, role)
            ...
            if Options then
                if Options.HitboxWhitelist and type(Options.HitboxWhitelist.SetValue) == 'function' then
                    pcall(function()
                        local current = Options.HitboxWhitelist.Value or {}
                        local newTable = {}
                        local changed = false
                        for k, v in pairs(current) do
                        ...
                        if changed then
                            Options.HitboxWhitelist:SetValue(newTable)
                        end
                    end)
                end
    ...
    ;(function()
        ...
        -- Panic Mode: while admin spectates you, force-off pSilent / Hitboxes
        ...
        do
                    ...
                    if should then
                        if not panicSaved then
                            panicSaved = {
                                ...
                                hitbox = State.HitboxEnable and State.HitboxEnable.Value == true,
                            }
                        end
                        setToggleSafe(State.AimLock and State.AimLock.Enabled, false)
                        setToggleSafe(State.HitboxEnable, false)
                        setToggleSafe(Toggles.HitboxEnable, false)
                    elseif panicSaved then
                        local saved = panicSaved
                        panicSaved = nil
                        setToggleSafe(State.AimLock and State.AimLock.Enabled, saved.silent)
                        setToggleSafe(State.HitboxEnable, saved.hitbox)
                        setToggleSafe(Toggles.HitboxEnable, saved.hitbox)
                    end
                end)
                panicApplying = false
            end
            ...
            attachChangeListener(State.HitboxEnable, guardWhilePanicked)
    ...
    local function setMenuVisible(show)
        ...
        if show then
            ...
            if dbHitboxUi.setVisible then
                dbHitboxUi.setVisible(false)
            end
            if revHitboxUi.setVisible then
                revHitboxUi.setVisible(false)
            end
            if sgHitboxUi.setVisible then
                sgHitboxUi.setVisible(false)
            end
            if tacHitboxUi.setVisible then
                tacHitboxUi.setVisible(false)
            end
...
trackConnection(safeConnect(UIS.InputEnded, function(input)
    ...
    Library:OnUnload(function()
        ...
        pcall(function()
            if dbHitboxUi.panel then
                dbHitboxUi.panel:Destroy()
            end
        end)
        pcall(function()
            if revHitboxUi.panel then
                revHitboxUi.panel:Destroy()
            end
        end)
        pcall(function()
            if sgHitboxUi.panel then
                sgHitboxUi.panel:Destroy()
            end
        end)
        pcall(function()
            if tacHitboxUi.panel then
                tacHitboxUi.panel:Destroy()
            end
        end)
        pcall(function()
            if missShotsUi.panel then
